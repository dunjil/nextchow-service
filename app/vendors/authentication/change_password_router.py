...
from app.vendors.schemas import PasswordResetRequestSchema, OTPVerification, PasswordResetSchema,ChangePasswordSchema
...
vendor_password_router = APIRouter(prefix="/vendor", tags=["Vendor Password Management"])
...
async def request_password_reset(
    ...
    try:
        # Find user by email
        user = await db[NEXTCHOW_COLLECTIONS.VENDOR_USER].find_one({"email": reset_request.email})
...
async def verify_password_reset_otp(
    ...
    try:
        # Find user by email
        user = await db[NEXTCHOW_COLLECTIONS.VENDOR_USER].find_one({"email": otp_verification.email})
...
async def reset_password(
    ...
    try:
        # Find user by email
        user = await db[NEXTCHOW_COLLECTIONS.VENDOR_USER].find_one({"email": password_reset.email})
...
async def change_password(
    ...
    try:
        # Find user by ID
        current_user = await db[NEXTCHOW_COLLECTIONS.VENDOR_USER].find_one({"_id": user.get("_id")})
