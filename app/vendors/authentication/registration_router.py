...
async def vendor_signup(
    ...
    try:
        ...
        existing_user = await db[NEXTCHOW_COLLECTIONS.VENDOR_USER].find_one({"email": signup_data.email})
        if existing_user:
            raise HTTPException(
                status_code=400, 
...
async def vendor_login(
    ...
    try:
        # Find user by email
        user = await db[NEXTCHOW_COLLECTIONS.VENDOR_USER].find_one({"email": login_data.email})
