...
from app.customers.schemas import PasswordResetRequestSchema, OTPVerification, PasswordResetSchema,ChangePasswordSchema
...
from app.general.utils.oauth_service import get_password_hash, verify_password
from app.general.utils.mail_sender import send_password_reset_otp

customer_password_router = APIRouter(prefix="/customer", tags=["Customer Password Management"])

@customer_password_router.post("/request-password-reset")
async def request_password_reset(
    ...
    try:
        # Find user by email
        user = await db[NEXTCHOW_COLLECTIONS.CUSTOMER_USER].find_one({"email": reset_request.email})
...
@customer_password_router.post("/verify-password-reset-otp")
async def verify_password_reset_otp(
    ...
    try:
        # Find user by email
        user = await db[NEXTCHOW_COLLECTIONS.CUSTOMER_USER].find_one({"email": otp_verification.email})
        ...
        if (datetime.now() - otp_created_at).total_seconds() > 900:
            await db[NEXTCHOW_COLLECTIONS.CUSTOMER_USER].update_one(
                ...
                {"$unset": {"password_reset_otp": 1, "password_reset_otp_created_at": 1}}
            )
            raise HTTPException(
                status_code=400, 
                detail={"success": False, "message": "OTP has expired"}
            )
...
@customer_password_router.post("/reset-password")
async def reset_password(
    ...
    try:
        # Find user by email
        user = await db[NEXTCHOW_COLLECTIONS.CUSTOMER_USER].find_one({"email": password_reset.email})
...
@customer_password_router.post("/change-password")
async def change_password(
    ...
    try:
        # Find user by ID
        current_user = await db[NEXTCHOW_COLLECTIONS.CUSTOMER_USER].find_one({"_id": user.get("_id")})
